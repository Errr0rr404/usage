...
const { sessionFromSecret } = require('./oauth.cjs');
...
async function fetchGemini(token) {
  const headers = {
    ...
    'Content-Type': 'application/json',
    Accept: 'application/json',
    'User-Agent': 'GeminiCLI/0.1.0 (usage)',
  };
  ...
  const project = loaded.cloudaicompanionProject || loaded.cloudaicompanion_project;
  if (!project) throw new ProviderError('Gemini did not attach a project to this login.', 404);
  const quota = await requestJson('https://cloudcode-pa.googleapis.com/v1internal:retrieveUserQuota', {
    method: 'POST',
    headers,
    body: JSON.stringify({ project }),
  }, token);
  const parsed = parseGeminiQuota(quota, loaded);
...
async function fetchUsage(account, secret) {
  try {
    ...
    const scoped = session.resourceUrl
      ...
      : account;
    let parsed;
    if (account.provider === 'claude') parsed = await fetchClaude(token);
    else if (account.provider === 'codex') parsed = await fetchCodex(scoped, token);
    else if (account.provider === 'grok') parsed = await fetchGrok(token);
    else if (account.provider === 'minimax') parsed = await fetchMiniMax(scoped, token);
    else if (account.provider === 'cursor') parsed = await fetchCursor(token);
    else if (account.provider === 'copilot') parsed = await fetchCopilot(token);
    else if (account.provider === 'gemini') parsed = await fetchGemini(token);
    else throw new ProviderError('Unknown service.', 400);
    return {
      ok: true,
      error: null,
    ...
    return {
      ok: false,
      error: messageFor(account.provider, error instanceof ProviderError ? error : Object.assign(error, {})),
      plan: null,
      identity: null,
      windows: [],
      fetchedAt: new Date().toISOString(),
