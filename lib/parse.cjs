...
function parseGeminiQuota(json, loaded = {}) {
  ...
  for (const bucket of buckets) {
    const remaining = asNumber(pick(bucket, 'remainingFraction', 'remaining_fraction'));
    if (remaining == null) continue;
    const fraction = remaining > 1 ? remaining / 100 : remaining;
    const model = String(pick(bucket, 'modelId', 'model_id') || 'Quota').split('/').pop();
    const label = model.replace(/^gemini-?/i, '').replace(/-/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase()) || 'Quota';
    windows.push(makeWindow({
      key: model || label,
      label,
